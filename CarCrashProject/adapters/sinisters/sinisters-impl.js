...
var selectStatement = WL.Server.createSQLStatement(
	...
	"RE.MedicalAssistance AS medicalAssistance, RE.LegalAssistance AS legalAssistance, RE.Comments AS comments " +
	"FROM Reports R " +
...
var addExtrasStatement = WL.Server.createSQLStatement(
	"INSERT INTO ReportsExtra (Identifier, Email, MedicalAssistance, LegalAssistance, Comments) " +
	"VALUES(?,?,?,?,?)"
);
...
function save(sinister){
	...
	if(sinister.type != "theft"){
		b = WL.Server.invokeSQLStatement({
			...
			parameters : [sinister.identifier, sinister.email, sinister.extras.medicalAssistance ? 1 : 0, sinister.extras.legalAssistance ? 1 : 0, sinister.extras.comments]
		});
